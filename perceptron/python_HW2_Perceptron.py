...
#         loops += 1
#         if loops >= 100:
#             print('100 loops reached, breaking')
#             break
        converged = True
        for i in range(n):
            x = np.hstack((xs[i], [1]))
            y = ys[i]
#             print('predict ' + str(np.sign(w.dot(x))))
#             print('ground  ' + str(y))
            if y * w.dot(x) <= 0:
                w = perceptronUpdate(x,y,w)
                converged = False
    b = w[-1]
    w = w[:-1]
    ## ... until here
...
def classifyLinear(xs,w,b=0):
    ...
    if n > 1:
        for index in range(n):
            preds[index] = np.sign(w.dot(xs[index])+b)
    else:
        preds[0] = np.sign(w.dot(xs)+b)
